...
import yaml
print("[okayiva] augmented features...", flush=True)
clips=Clips(input_directory='generated_samples_okayiva', file_pattern='*.wav', max_clip_duration_s=None,
...
os.makedirs("generated_augmented_features_okayiva", exist_ok=True)
for split in ["training","validation","testing"]:
    out=f"generated_augmented_features_okayiva/{split}"; os.makedirs(out, exist_ok=True)
...
config={"window_step_ms":10,"train_dir":"trained_models/wakeword_okayiva","features":[
...
yaml.dump(config, open("training_parameters_okayiva.yaml","w"))
print("[okayiva] DONE features+config", flush=True)
